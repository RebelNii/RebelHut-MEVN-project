<template>
    <div id="nav" class="relative bg-slate-800 text-white p-2 mb-10">
        ...
        <div class="em mt-2 flex justify-between items-center px-5">
            ...
            <div class=" px-4">
                <h5 class="flex justify-end items-center text-sm md:text-base font-medium">
                <a href="javascript:void(0)" @click="logouts" class="bg-red-300 p-1 rounded hover:bg-red-600" v-if="this.$store.state.active">Logout</a> <router-link v-else class="bg-green-400 rounded p-1" :to="{name:'login'}">Login</router-link> 
            </h5>
            </div>
        </div>
    </div>
